...
# akeman小车的参数，以两个后轮中心为原点，小车前进方向为x轴正方向
# 遵守右手定则
...
BUBBLE_R = np.hypot((LF + LB) / 2.0, W / 2.0)  # bubble radius, hypotenuse斜边，计算

# vehicle rectangle vertices。车辆前方为x轴正方向，左方为y轴正方向，右手定则。以两后轮的中心为原点。
VRX = [LF, LF, -LB, -LB, LF]
...
def check_car_collision(x_list, y_list, yaw_list, ox, oy, kd_tree):
    for i_x, i_y, i_yaw in zip(x_list, y_list, yaw_list):
        ...
        ids = kd_tree.query_ball_point([cx, cy], BUBBLE_R)  # 在obstacle中寻找与车辆中心距离为BUBBLE_R的障碍物。
